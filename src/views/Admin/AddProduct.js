...
//         try {
//             const response = await axios.get(`http://localhost:9999/api/productById/${id}`);
...
//             ? `http://localhost:9999/api/edit_product/${productId}`
//             : 'http://localhost:9999/api/create_product';
...
const BASE_URL = 'http://134.209.145.149:9999/api';
    const [formData, setFormData] = useState({
        name: '',
        description: '',
        category: '',
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const apiUrl = isEditMode
            ? `${BASE_URL}/edit_product/${productId}`
        ...
        try {
            ...
            Object.keys(formData).forEach(key => {
                if (key === 'dimensions' || key === 'tags' || key === 'variations') {
